...
contract FooFighters is ERC721, Owned {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory attributes = string.concat(
            ...
            tokenHealth.toString(),
            "}]"
        );
        string memory metadataString = string.concat(
            '{"name": "FooFighter #',
            tokenId.toString(),
            '", "description": "FooFighters are a fun onchain experiment", ',
            attributes,
            ', "image": "',
            svgEncoded,
            '"}'
